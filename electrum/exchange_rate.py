...
class CoinPaprika(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        json = await self.get_json('api.coinpaprika.com',
                                   '/v1/tickers/kiiro-kiirocoin?quotes=USD,BTC,EUR')
        
        if json is None:
            return {"USD": Decimal(0.0), "BTC": Decimal(0.0), "EUR": Decimal(0.0) }

        r = {"USD": Decimal(json['quotes']['USD']['price']),"BTC": Decimal(json['quotes']['BTC']['price']), "EUR": Decimal(json['quotes']['EUR']['price']) }
        return r
...
class Xeggex(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        json = await self.get_json('xeggex.com',
                                   '/api/v2/ticker/KIIRO_'+ccy.upper()+'T')
        
        if json is None:
            return {"USD": Decimal(0.0)}

        r = {"USD": Decimal(json['last_price'])}
        return r
    ...
    async def request_history(self, ccy):
        history = await self.get_json('xeggex.com',
                                      '/api/v3/coins/zcoin/market_chart?vs_currency=%s&days=max' % ccy)
