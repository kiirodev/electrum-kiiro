...
class ElectrumWindow(App, Logger):
    ...
    def on_new_intent(self, intent):
        ...
        if scheme in [KIIRO_BIP21_URI_SCHEME, PAY_BIP21_URI_SCHEME]:
            self.set_URI(data)
    ...
    def scan_qr(self, on_complete):
        ...
        PythonActivity = autoclass('org.kivy.android.PythonActivity')
        SimpleScannerActivity = autoclass("org.kiirocoin.electrum.qr.SimpleScannerActivity")
        Intent = autoclass('android.content.Intent')
    ...
    def notify(self, message):
        try:
            ...
            icon = (os.path.dirname(os.path.realpath(__file__))
                    + '/../../' + self.icon)
            notification.notify('Kiirocoin Electrum', message,
                            app_icon=icon, app_name='Kiirocoin Electrum')
        except ImportError:
